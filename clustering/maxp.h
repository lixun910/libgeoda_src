...
public:
     ...
     */
    Maxp(const GalElement* w, const vector<vector<double> >& z, double floor,
            double* floor_variable, int initial, vector<uint64_t> seeds,
            int _method, int _tabu_lenght, double _cool_rate,
            int rnd_seed=-1, char dist='e',  bool test=false);
...
protected:
    ...
    std::string print_regions(vector<vector<int> >& _regions);
    //! xxx
    /* !
     \param block
     \param neighbor
