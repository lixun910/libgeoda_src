...
public:
    ...
    static double ManhattanDist(const std::vector<double>& x1,
    ...
    static std::vector<std::vector<double> >  DistanceMatrix(const std::vector<std::vector<double> >& data,
            char dist_method)
    {
        ...
        if (ncols == 0) return matrix;

        matrix.resize(nrows);
        for (size_t i=0; i<nrows; ++i) matrix[i].resize(nrows, 0);
